...
public class ProjectRefSettingPageTester implements IProjectSettingPageTester {
    ...
    public boolean valid(IConfigurationElement element, IPreferenceNode node) {
        if (PluginChecker.isRefProjectLoaded() && !ProjectManager.getInstance().getCurrentProject().isLocal()) {
            return true;
        }
        return false;
    }
