...
public class AddEncodingToPostParametersFortFileFetchMigrationTask extends AbstractJobMigrationTask {
	...
	public Date getOrder() {
		GregorianCalendar gc = new GregorianCalendar(2021, 3, 26, 18, 0, 0);
		return gc.getTime();
	}
	...
	public ExecutionResult execute(Item item) {
		...
		String[] compNames = { "tFileFetch" }; //$NON-NLS-1$
		...
		IComponentConversion conversion = new IComponentConversion() {
			...
			public void transform(NodeType node) {
				...
				ElementParameterType parameter = ComponentUtilities
						.getNodeProperty(node, "COMMON_PARAMS_CHARSET"); //$NON-NLS-1$
