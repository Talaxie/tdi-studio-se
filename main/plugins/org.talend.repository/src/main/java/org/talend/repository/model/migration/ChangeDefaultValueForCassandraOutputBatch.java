...
public class ChangeDefaultValueForCassandraOutputBatch extends
	...
	public Date getOrder() {
		GregorianCalendar gc = new GregorianCalendar(2016, 4, 16, 12, 0, 0);
		return gc.getTime();
	}
		...
		IComponentConversion conversion = new IComponentConversion() {
			...
			public void transform(NodeType node) {
				...
				ElementParameterType parameter = ComponentUtilities
						.getNodeProperty(node, "ASYNC"); //$NON-NLS-1$

				if (parameter == null) {
					ComponentUtilities.addNodeProperty(node,
							"ASYNC", "CHECK"); //$NON-NLS-1$ //$NON-NLS-2$
					ComponentUtilities.setNodeValue(node,
							"ASYNC", "false"); //$NON-NLS-1$ //$NON-NLS-2$
				}
			}
