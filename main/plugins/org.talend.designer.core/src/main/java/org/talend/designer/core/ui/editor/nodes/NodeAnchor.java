...
public class NodeAnchor extends ChopboxAnchor {
	...
	public Point getLocation(Point reference) {
		if (target != null && !target.equals(source)) {
			...
			/*
			 * Use owner to translate the locations and rectangles. This ensures we use
			 * correct scale, including with High DPI (windows scaling) and have the correct
			 * coordinates translated relatively.
			 */
			getOwner().translateToAbsolute(sourceRect);
			getOwner().translateToAbsolute(targetRect);
	...
	public Point getLocationForMultipleConnections(int connectionId) {
		if ((sourceLocation.y >= targetLocation.y)
			...
			targetRect = new Rectangle(new Point(targetLocation.x, sourceLocation.y),
					new Dimension(targetRect.width, sourceRect.height));
		} else if ((sourceLocation.x >= targetLocation.x)
				&& ((sourceLocation.x + sourceRect.width) <= (targetLocation.x + targetRect.width))) {
			targetRect = new Rectangle(new Point(sourceLocation.x, targetLocation.y),
					new Dimension(sourceRect.width, sourceRect.height));
		}

		/*
		 * will calculate the numerator and denominator for the function to place the
		 * points. example: 1/2; 3/4; 1/4; 5/8; 3/8; 7/8; 1/8; 9/16; 7/16; 11/16; 5/16;
		 * 13/16; 3/16...
		 */
		int num = 1;
		int numPair = 1;
