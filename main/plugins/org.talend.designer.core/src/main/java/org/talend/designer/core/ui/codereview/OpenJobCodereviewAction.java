...
/**
 * This action opens a job codereview on the selected job.
 * <p>
 ...
 */
public class OpenJobCodereviewAction extends SelectionAction {

    // private JavaEditor fEditor;

    /**
     * Creates a new <code>OpenJobCodereviewAction</code>. The action requires that the selection provided by the site's
     * selection provider is of type <code>
     * org.eclipse.jface.viewers.IStructuredSelection</code>.
     *
     * @param site the site providing context information for this action
     */
    public OpenJobCodereviewAction(IWorkbenchPart part) {
        super(part);
        setText(Messages.getString("OpenJobCodereviewAction.textContent")); //$NON-NLS-1$
        setToolTipText(Messages.getString("OpenJobCodereviewAction.tipTextContent")); //$NON-NLS-1$
        setDescription(Messages.getString("OpenJobCodereviewAction.description")); //$NON-NLS-1$
    }
    ...
    private static void openInViewPart(IWorkbenchWindow window, IProcess2 input) {
        ...
        try {
            JobCodereviewViewPart result = (JobCodereviewViewPart) page.findView(JobCodereviewViewPart.ID);
            // if (result != null) {
            // result.clearNeededRefresh(); // avoid refresh of old codereview on 'becomes visible'
            // }
            result = (JobCodereviewViewPart) page.showView(JobCodereviewViewPart.ID);
