...
class TernarySearchTree {
	...
	private static final int MAX_MATCH_ALMOST_DIFF = 4;
	...
	*/
	...
	private int maxMatchAlmostDiff = MAX_MATCH_ALMOST_DIFF;				// convenience variable for matchAlmost
	...
	*/
	protected TSTNode getOrCreateNode(String key) {
		if (key == null) {
            throw new NullPointerException("attempt to get or create node with null key"); //$NON-NLS-1$
        }
		if (key.length() == 0) {
            throw new IllegalArgumentException("attempt to get or create node with key of zero length"); //$NON-NLS-1$
        }
		if (rootNode == null) {
